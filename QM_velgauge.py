...
class vecpot:
    def __init__(self, dt, omega , amplitude=1.0):
        #self.field_type = field_type
        self.amplitude = amplitude
        self.dt = dt
        self.omega = omega
    ...
    def generate(self, t):
        t0= 1000*self.dt
        #add damping function
        return -self.amplitude/self.omega * np.sin(self.omega * t)*2/np.pi* np.arctan(t/t0)
...
class QM:
    ...
    def update_vel(self, PsiRe, PsiIm, dy, dt, hbar, m, q, r, potential, pot, n,order,N):
        #E = efield.generate((n)*dt)*np.ones(Ny)
        #E = efield.generate((n)*dt)*np.ones(Ny)
        #E = efield.generate((n)*dt, omega=omegaHO)*np.ones(Ny)
        a = pot.generate(n*dt)
        ...
        #E= 0
        PsiReo = PsiRe
        PsiRe = PsiReo -hbar*dt/(2*m)*self.diff(PsiIm,dy,order) + dt/hbar*(potential.V())*PsiIm +dt*q/m*a/dy*(np.roll(PsiRe,-1)-np.roll(PsiRe,1))
        #PsiRe = PsiRe -hbar*dt/(2*m)*self.diff(PsiIm,dy,order) - dt/hbar*(-potential.V())*PsiIm
        a = pot.generate((n+1/2)*dt)
        PsiRe[0] = 0
        PsiRe[-1] = 0
        #E = efield.generate((n+1/2)*dt)*np.ones(Ny)
        #E = efield.generate((n+1/2)*dt)*np.ones(Ny)
        ...
        #E= 0
        PsiImo = PsiIm
        PsiIm = PsiImo +hbar*dt/(2*m)*self.diff(PsiRe,dy,order) - dt/hbar*(potential.V())*PsiRe +dt*q/m*a/dy*(np.roll(PsiIm,-1)-np.roll(PsiIm,1))
        #PsiIm = PsiIm +hbar*dt/(2*m)*self.diff(PsiRe,dy, order) + dt/hbar*(-potential.V())*PsiRe
    ...
    def calc_wave(self, dy, dt, Ny, Nt,  hbar, m ,q ,potential, a,alpha,order,N):
        ...
        for n in range(1, Nt):
            PsiRe, PsiIm , J ,prob, momentum = self.update_vel(PsiRe, PsiIm, dy, dt, hbar, m, q, r, potential, pot, n,order,N)
    ...
    def expvalues(self,dt, dy,  type):
        if self.result == None:
            data_time, dataRe, dataIm, dataprob, datacurr , datamom = self.calc_wave( dy, dt, Ny, Nt,  hbar, m ,q ,potential, pot,alpha,order,N)
...
pot= vecpot(dt, omegaHO, amplitude=1e9)
...
qm = QM(order)
res = qm.calc_wave( dy, dt, Ny, Nt,  hbar, m ,q ,potential, pot,alpha, order,N)
#res = qm.calc_wave( dy, dt, Ny, Nt,  hbar, m ,q ,potential, Efield, omegaHO, alpha, order,N)
# prob = res[3]
# probsel = prob[::100]

qm.animate( dy, dt, Ny, Nt,  hbar, m ,q ,potential, pot,alpha,order,N)
# plt.imshow(probsel)
# plt.colorbar()
# #plt.plot(prob[8000])
# plt.show()
...
qm.heatmap(dy, dt, Ny, Nt,  hbar, m ,q ,potential, pot,alpha,order,N)