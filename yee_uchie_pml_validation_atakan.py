...
class Yee_UCHIE:
    ...
    def uchie_update(self):
        ...
        for QMw in self.QMwires:
            ...
            slice = int(1/2*(self.ny-self.NyQM))
            Y[QMw.QMxpos, slice :-slice]+= -2 * (1 / Z0)
    ...
    def validation(self, recorder, source, dx, dy):
        ...
        padding = 100000
        freq_axis, FT = self.fourier(Hz, omega_max, padding, self.dt)
...
test = Yee_UCHIE(Nx, Ny, Nt, dx, dy, dt, Ly, n, N_sub, NyQM, x_sub1, x_sub2, eps0, mu0, source, QMscheme1 , QMscheme2, QMxpos1, QMxpos2, recorders=recorders)
...
# plt.plot(recorder1.data_time, recorder1.data)
# plt.show()
# plt.close()

test.validation(recorder1, source, dx, dy)
