...
class PML_Y:
    def __init__(self,  Nx, Ny, dx, dy, dt,reverse = False, pml_kmax = None, pml_nl = None):
        ...
        self.dy = dy
        self.dt = c0*dt
        self.X = np.zeros((3*Nx+3, Ny)) # the first Nx+1 rows are the Ey fields, and the others the Bz fields
        ...
        for j in range(start, stop, step):
            ...
            if j < Ny-pml_nl:
                k_tot_H = np.ones(Nx+1)
                sigma_tot_H = np.zeros(Nx+1)
            else:
                
                k_tot_H = np.full((Nx+1,), 1 + (pml_kxmax -1)*((j-Ny+pml_nl)/pml_nl)**m)
                sigma_tot_H = np.full((Nx+1,), pml_sigmax_max*((j-Ny+pml_nl)/pml_nl)**m )
            ...
            M1 = np.hstack((A1/self.dt,                  D2,                          np.zeros((Nx+2, Nx+1)) ))
            M2 = np.hstack(( D1,                        A2/self.dt,                   np.zeros((Nx, Nx+1))   ))
            M3 = np.hstack((np.zeros((Nx + 1, Nx+1)),    I_H/self.dt,       np.diag(k_tot_H/self.dt+Z0*sigma_tot_H/2) ))

            N1 = np.hstack((A1/self.dt,                  -D2,                          np.zeros((Nx+2, Nx+1)) ))
            N2 = np.hstack(( -D1,                        A2/self.dt,                   np.zeros((Nx, Nx+1))   ))
            N3 = np.hstack((np.zeros((Nx + 1, Nx+1)),    I_H/self.dt,       np.diag(k_tot_H/self.dt-Z0*sigma_tot_H/2) ))

            M = np.vstack((M1, M2, M3))

            self.M_inv.append(np.linalg.inv(M))
            self.N.append(np.vstack((N1, N2, N3)))
            self.M_N.append(self.M_inv[-1]@self.N[-1])

            self.Betay_min.append(np.diag(k_tot_H/self.dt-Z0*sigma_tot_H/2))
            self.Betay_plus_inv.append(np.linalg.inv(np.diag(k_tot_H/self.dt+Z0*sigma_tot_H/2)))
        ...
        self.Y =   np.vstack((np.zeros((self.Nx+2, self.Ny)),self.A2@(self.ex0[:, 1:] - self.ex0[:, :-1])/self.dy, np.zeros((self.Nx+1, self.Ny))))
